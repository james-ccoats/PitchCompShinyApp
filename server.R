# server.R - Baseball Pitch Finder Shiny App Server Logic
library(shiny)
library(dplyr)
library(DT)

# Helper function: clock string to degrees
clock_to_spinaxis <- function(clock_str) {
  parts <- strsplit(clock_str, ":")[[1]]
  hr <- as.numeric(parts[1]) %% 12
  mn <- as.numeric(parts[2])
  deg_clock <- (hr * 30) + (mn / 60) * 30
  spin_axis <- (deg_clock - 180) %% 360
  spin_axis
}

# Helper function to get display name from Name column or pitcher_id
get_display_name <- function(name_col, pitcher_id) {
  if (!is.na(name_col) && name_col != "" && !is.null(name_col)) {
    return(name_col)
  } else {
    return("pitcher name not found")
  }
}

# Function to add pitcher names to dataset (updated for Name column)
add_pitcher_names <- function(data) {
  if ("Name" %in% names(data)) {
    # Use existing Name column and create pitcher_name for consistency
    data <- data %>%
      mutate(
        pitcher_name = get_display_name(Name, pitcher_id)
      )
  } else if ("pitcher_name" %in% names(data)) {
    return(data)  # Already has pitcher_name
  } else {
    # No name column available
    data <- data %>%
      mutate(
        pitcher_name = "pitcher name not found"
      )
  }
  
  return(data)
}

# Define server logic
function(input, output, session) {
  
  # Reactive values to store results
  values <- reactiveValues(
    results = NULL,
    closest_pitch = NULL,
    search_performed = FALSE
  )
  
  # Check if data exists
  data_available <- reactive({
    exists("savant_data_mod") && is.data.frame(savant_data_mod)
  })
  
  # Search function
  observeEvent(input$search_btn, {
    
    if (!data_available()) {
      values$search_performed <- TRUE
      output$status_message <- renderText({
        "Error: 'savant_data_mod' dataset not found. Please load your data first."
      })
      return()
    }
    
    # Validate tilt input
    tilt_valid <- grepl("^[0-9]{1,2}:[0-9]{2}$", input$tilt_input)
    if (!tilt_valid) {
      output$status_message <- renderText({
        "Error: Tilt must be in format 'H:MM' (e.g., '1:30', '12:00')"
      })
      return()
    }
    
    tryCatch({
      target_spin_axis <- clock_to_spinaxis(input$tilt_input)
      
      # Add pitcher names to dataset
      data_with_names <- add_pitcher_names(savant_data_mod)
      
      # Filter data
      filtered_data <- data_with_names %>%
        filter(
          pitch_hand == input$handedness_input,
          tolower(pitch_name) == input$pitch_type_input,
          release_spin_rate >= input$rpm_input
        )
      
      if (nrow(filtered_data) == 0) {
        values$search_performed <- TRUE
        output$status_message <- renderText({
          "No pitches found matching the basic filters (handedness, pitch type, RPM)."
        })
        return()
      }
      
      # Find closest pitch
      closest_pitch <- filtered_data %>%
        mutate(
          pfx_x_diff = abs(pfx_x * -12 - input$pfx_x_input),
          pfx_z_diff = abs(pfx_z * 12 - input$pfx_z_input),
          vaa_diff = abs(VAA - input$vaa_input),
          tilt_diff = abs(((spin_axis - target_spin_axis + 180) %% 360) - 180),
          total_diff = pfx_x_diff + pfx_z_diff + vaa_diff + tilt_diff
        ) %>%
        arrange(total_diff) %>%
        slice(1)
      
      if (nrow(closest_pitch) > 0) {
        values$closest_pitch <- closest_pitch
        values$results <- filtered_data %>%
          mutate(
            pfx_x_diff = abs(pfx_x * -12 - input$pfx_x_input),
            pfx_z_diff = abs(pfx_z * 12 - input$pfx_z_input),
            vaa_diff = abs(VAA - input$vaa_input),
            tilt_diff = abs(((spin_axis - target_spin_axis + 180) %% 360) - 180),
            total_diff = pfx_x_diff + pfx_z_diff + vaa_diff + tilt_diff
          ) %>%
          arrange(total_diff) %>%
          slice_head(n = 10)  # Top 10 matches
        
        values$search_performed <- TRUE
        output$status_message <- renderText({
          paste("Search completed successfully!", 
                nrow(filtered_data), "pitches matched basic filters.")
        })
      } else {
        values$search_performed <- TRUE
        output$status_message <- renderText({
          "No matching pitches found after applying all filters."
        })
      }
      
    }, error = function(e) {
      values$search_performed <- TRUE
      output$status_message <- renderText({
        paste("Error during search:", e$message)
      })
    })
  })
  
  # Output for results availability
  output$results_available <- reactive({
    !is.null(values$closest_pitch)
  })
  outputOptions(output, "results_available", suspendWhenHidden = FALSE)
  
  # Pitch summary output
  output$pitch_summary <- renderText({
    if (!is.null(values$closest_pitch)) {
      cp <- values$closest_pitch
      sprintf(
        paste(
          "Pitcher: %s",
          "Pitcher ID: %s",
          "Pitch Name: %s", 
          "RPM: %d",
          "Horizontal Break: %.2f inches (%.2f off target)",
          "Vertical Break: %.2f inches (%.2f off target)",
          "VAA: %.2f° (%.2f off target)",
          "Tilt: %s (%.1f° off spin-axis)",
          sep = "\n"
        ),
        if("pitcher_name" %in% names(cp)) cp$pitcher_name else get_display_name(cp$Name, cp$pitcher_id),
        cp$pitcher_id,
        cp$pitch_name,
        cp$release_spin_rate,
        cp$pfx_x * -12, cp$pfx_x_diff,
        cp$pfx_z * 12, cp$pfx_z_diff,
        cp$VAA, cp$vaa_diff,
        if("tilt" %in% names(cp)) cp$tilt else "N/A", cp$tilt_diff
      )
    }
  })
  
  # Results table
  output$results_table <- DT::renderDataTable({
    if (!is.null(values$results)) {
      # Use pitcher_name (which is created from Name column) for display
      pitcher_col <- if("pitcher_name" %in% names(values$results)) "pitcher_name" else "pitcher_id"
      
      display_cols <- c(pitcher_col, "pitcher_id", "pitch_name", "release_spin_rate", 
                        "pfx_x", "pfx_z", "VAA", "spin_axis", "total_diff")
      
      # Only include columns that exist
      available_cols <- intersect(display_cols, names(values$results))
      
      results_display <- values$results %>%
        select(all_of(available_cols)) %>%
        mutate(
          pfx_x = round(pfx_x * -12, 2),
          pfx_z = round(pfx_z * 12, 2),
          VAA = round(VAA, 2),
          total_diff = round(total_diff, 2)
        )
      
      # Rename columns for better display
      if ("pitcher_name" %in% names(results_display)) {
        names(results_display)[names(results_display) == "pitcher_name"] <- "Pitcher Name"
      }
      if ("pitcher_id" %in% names(results_display)) {
        names(results_display)[names(results_display) == "pitcher_id"] <- "Pitcher ID"
      }
      
      names(results_display) <- gsub("pitch_name", "Pitch", names(results_display))
      names(results_display) <- gsub("release_spin_rate", "RPM", names(results_display))
      names(results_display) <- gsub("pfx_x", "H-Break", names(results_display))
      names(results_display) <- gsub("pfx_z", "V-Break", names(results_display))
      names(results_display) <- gsub("spin_axis", "Spin Axis", names(results_display))
      names(results_display) <- gsub("total_diff", "Total Diff", names(results_display))
      
      results_display
    }
  }, options = list(pageLength = 10, scrollX = TRUE))
  
  # Video button functionality
  observeEvent(input$view_video, {
    if (!is.null(values$closest_pitch) && "play_id" %in% names(values$closest_pitch)) {
      # Check if required packages are available
      if (requireNamespace("httr", quietly = TRUE) && 
          requireNamespace("sabRmetrics", quietly = TRUE)) {
        tryCatch({
          httr::BROWSE(sabRmetrics::get_video_url(values$closest_pitch$play_id[1]))
        }, error = function(e) {
          showNotification("Error opening video: Package 'sabRmetrics' may not be available", 
                           type = "error")
        })
      } else {
        showNotification("Video functionality requires 'httr' and 'sabRmetrics' packages", 
                         type = "warning")
      }
    } else {
      showNotification("No video available for this pitch", type = "warning")
    }
  })
}